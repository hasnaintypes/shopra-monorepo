...
A robust, reusable Kafka client for Node.js microservices, providing unified producer, consumer, and admin abstractions. Built for monorepo architectures, it leverages centralized configuration and logging, and is compatible with both ESM and CJS environments.

---

## Features

- **Unified Kafka Producer, Consumer, and Admin**: Simple, type-safe APIs for all core Kafka operations.
- **Centralized Configuration**: Loads Kafka connection and topic settings from the shared `@shopra/config` package.
- **Integrated Logging**: Uses the shared `@shopra/logging` library for consistent, structured logs.
- **TypeScript-first**: Full type safety and autocompletion via shared types.
...
All logs are routed through the shared `@shopra/logging` library, ensuring consistent, structured, and centralized logging across all services.
